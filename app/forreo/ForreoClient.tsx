...
function FloatingEmojis({ hearts, petals }: { hearts: Heart[]; petals: Heart[] }) {
  return (
    <>
      {petals.map(p => (
        <div
...
// ─── Love note card ───────────────────────────────────────────────────────────
function NoteCard({ loaded, clicked, onActivate }: {
  loaded: boolean
  clicked: boolean
  onActivate: () => void
}) {
    ...
    <div style={{
      ...
      <div
        ...
        style={{
          ...
          border: '1px solid rgba(196,20,40,0.25)', borderRadius: '24px',
      ...
      >
        ...
        <div style={{ fontFamily: 'Pinyon Script, cursive', fontSize: '52px', color: '#f0a0b8', lineHeight: 1.1, marginBottom: '24px', textShadow: '0 0 30px rgba(196,20,40,0.4)' }}>
          for you
        </div>

        <p style={{ fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', fontSize: '19px', lineHeight: 1.75, color: 'rgba(245,232,255,0.92)', letterSpacing: '0.02em', marginBottom: '32px' }}>
          hai sweetie 🩷<br />
          <br />
        ...
        <div style={{ fontFamily: 'Pinyon Script, cursive', fontSize: '28px', color: 'rgba(196,20,40,0.6)', marginBottom: '32px' }}>
          — with all my love
        </div>
        ...
        <button type="button" onClick={onActivate} className={`magic-btn${clicked ? ' activated' : ''}`}>
          {clicked ? '💝 i love you honey pie 💝' : '✨ press me ✨'}
        </button>

        {clicked && (
          <p style={{ marginTop: '20px', fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', fontSize: '15px', color: 'rgba(240,160,184,0.8)', animation: 'note-appear 0.6s ease both' }}>
      ...
      <a
        href="/"
        style={{ marginTop: '28px', fontFamily: 'Space Mono, monospace', fontSize: '11px', letterSpacing: '0.1em', color: 'rgba(245,232,255,0.25)', textDecoration: 'none', transition: 'color 0.2s ease' }}
    ...
    <main style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden', background: '#030009' }}>
      <style>{`
        ...
        @keyframes petal-fall {
          ...
          10% { opacity: 0.8; }
          90% { opacity: 0.6; }
          100% { transform: translateY(105vh) rotate(var(--rot)) translateX(40px); opacity: 0; }
        }
        @keyframes vignette-pulse { 0%, 100% { opacity: 0.7; } 50% { opacity: 0.85; } }
        ...
        .heart-particle {
          position: fixed; bottom: -10px; pointer-events: none; z-index: 100;
          font-size: var(--size);
          animation: heart-rise var(--dur) cubic-bezier(0.25, 0.46, 0.45, 0.94) var(--delay) forwards;
        }
        .petal {
          position: fixed; top: 0; pointer-events: none; z-index: 5;
          font-size: var(--size);
          animation: petal-fall var(--dur) ease-in var(--delay) forwards;
        }
        .note-card { animation: note-appear 1.2s cubic-bezier(0.16, 1, 0.3, 1) 0.3s both; }
        .magic-btn {
          ...
          background-size: 200% auto; color: #fef0f4;
          transition: transform 0.2s ease, box-shadow 0.2s ease;
          animation: button-shimmer 3s linear infinite; letter-spacing: 0.05em;
        }
        .magic-btn:hover { transform: scale(1.05); box-shadow: 0 0 30px rgba(196,20,40,0.55), 0 0 60px rgba(240,160,184,0.2); }
        .magic-btn:active { transform: scale(0.98); }
        .magic-btn.activated { animation: button-shimmer 1s linear infinite; box-shadow: 0 0 40px rgba(196,20,40,0.65), 0 0 80px rgba(240,160,184,0.25); }
      `}</style>

      <ForreoBackground />
      <FloatingEmojis hearts={hearts} petals={petals} />
      <NoteCard loaded={loaded} clicked={clicked} onActivate={handleClick} />
    </main>
  )
}
