...
function Countdown({ endsAt }: { endsAt: string }) {
  ...
  return (
    <span style={{ ...S, fontSize: 12, color: left === 'terminado' ? 'var(--primary)' : '#fbbf24', display: 'flex', alignItems: 'center', gap: 4 }}>
    ...
    >
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 8 }}>
          ...
          <span style={{
            ...S, fontSize: 12, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '3px 10px',
        ...
        {raffle.description && (
          <div style={{
            ...S, fontSize: 12, color: 'rgba(254,240,244,0.45)', lineHeight: 1.6, marginBottom: 14,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
          <div style={{ display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {raffle.prizes.length > 0 && (
              <span style={{ ...S, fontSize: 12, color: 'rgba(196,20,40,0.7)' }}>
            ...
            {!isActive && raffle.endsAt && (
              <span style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.25)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {raffle.winnerId && (
              <span style={{ ...S, fontSize: 12, color: '#ffd700' }}>🏆 {raffle.winnerId}</span>
            )}
            <span style={{ ...S, fontSize: 12, color: hovered ? 'rgba(196,20,40,0.8)' : 'rgba(254,240,244,0.2)', transition: 'color 0.2s', letterSpacing: '0.06em' }}>
              see →
            </span>
          </div>
        </div>
        ...
        {raffle.prizes.length > 0 && raffle.prizes.length <= 4 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 12 }}>
            {raffle.prizes.map((p, i) => (
              <span key={p.id} style={{
                ...S, fontSize: 12, padding: '3px 10px', borderRadius: 20,
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--bg)', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 560, margin: '0 auto', padding: '40px 20px 80px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, marginBottom: 6 }}>
            ...
            {connected && hasActive && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, paddingBottom: 6 }}>
                ...
                <span style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>on live</span>
              </div>
            )}
          </div>
          <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)', letterSpacing: '0.08em' }}>
        ...
        <div style={{ display: 'flex', gap: 3, background: 'rgba(255,255,255,0.03)', borderRadius: 11, padding: 3, marginBottom: 20, border: '1px solid rgba(255,255,255,0.05)' }}>
          {(['active', 'ended'] as const).map(t => (
            <button type="button"
              ...
              onClick={() => setTab(t)}
              style={{
                ...S, flex: 1, fontSize: 12, letterSpacing: '0.1em', textTransform: 'uppercase',
        ...
        {/* Content */}
        {loading ? (
          <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.25)', textAlign: 'center', padding: '60px 0', letterSpacing: '0.1em' }}>
            loading…
          </div>
        ) : displayed.length === 0 ? (
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(196,20,40,0.15)', borderRadius: 16, padding: '60px 24px', textAlign: 'center' }}>
            ...
            <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.2)', letterSpacing: '0.08em' }}>
