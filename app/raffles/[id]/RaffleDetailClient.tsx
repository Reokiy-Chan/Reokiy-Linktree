...
function Countdown({ endsAt }: { endsAt: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <span style={{ ...S, fontSize: 12, color: left === 'completed' ? 'var(--primary)' : '#fbbf24', letterSpacing: '0.06em' }}>
        {left}
      </span>
    </div>
  )
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', overflow: 'visible' }}>
        <div style={{ ...S, fontSize: 12, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(74,222,128,0.7)', marginBottom: 18, animation: 'jc-rise 0.4s ease both' }}>
          🎲 you&apos;re in!
        </div>
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 22 }}>
            <span style={{ ...S, fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(254,240,244,0.35)' }}>giveaway ticket</span>
          ...
          <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>good luck ✦</div>
        </div>
      </div>
    ...
    <main style={{ minHeight: '100vh', background: 'var(--bg)', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 560, margin: '0 auto', padding: '40px 20px 80px' }}>
        ...
        <button type="button"
          ...
          style={{ ...S, display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(254,240,244,0.3)', fontSize: 12, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '0 0 32px 0', transition: 'color 0.2s' }}
        ...
        {loading ? (
          <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)', textAlign: 'center', padding: '80px 0', letterSpacing: '0.1em' }}>loading…</div>
        ) : notFound || !raffle ? (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)', letterSpacing: '0.08em' }}>this giveaway doesn't exist or already ended</div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, animation: 'fadeInUp 0.4s ease both' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${isActive ? 'rgba(196,20,40,0.18)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 20, padding: '26px 26px 22px', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 14, marginBottom: 12 }}>
                ...
                <span style={{
                  ...S, fontSize: 12, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '4px 12px',
              ...
              <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', alignItems: 'center' }}>
                <span style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.45)' }}>
                  ...
                  {isActive && <span style={{ color: '#4ade80', marginLeft: 4, fontSize: 12 }}>●</span>}
                </span>
                ...
                {raffle.endsAt && !isActive && (
                  <span style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)' }}>
            ...
            {raffle.description && (
              <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 16, padding: '18px 20px' }}>
                <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 10 }}>Description</div>
            ...
            {raffle.prizes.length > 0 && (
              <div style={{ background: 'rgba(196,20,40,0.04)', border: '1px solid rgba(196,20,40,0.14)', borderRadius: 16, padding: '18px 20px' }}>
                <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 14 }}>REWARDS</div>
                  ...
                  {raffle.prizes.map((p, i) => (
                    <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(196,20,40,0.18)', border: '1px solid rgba(196,20,40,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <span style={{ ...S, fontSize: 12, color: 'var(--primary)', fontWeight: 'bold' }}>{i + 1}</span>
                      </div>
                      <div>
                        <div style={{ ...S, fontSize: 13, color: 'rgba(254,240,244,0.88)' }}>{p.label}</div>
                        {p.description && <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.35)', marginTop: 2 }}>{p.description}</div>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {raffle.winnerId && (
              <div style={{ background: 'rgba(255,215,0,0.06)', border: '1px solid rgba(255,215,0,0.2)', borderRadius: 16, padding: '20px 22px' }}>
                <div style={{ ...S, fontSize: 12, color: 'rgba(255,215,0,0.5)', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 12 }}>🏆 winner</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div style={{ fontSize: 36 }}>🏆</div>
                  <div style={{ ...S, fontSize: 22, color: '#ffd700', letterSpacing: '0.04em' }}>{raffle.winnerId}</div>
                </div>
                {raffle.pickedAt && (
                  <div style={{ ...S, fontSize: 12, color: 'rgba(255,215,0,0.4)', marginTop: 8 }}>
            ...
            {isActive && !alreadyEntered && (
              <div style={{ background: 'rgba(196,20,40,0.05)', border: '1px solid rgba(196,20,40,0.22)', borderRadius: 16, padding: '22px 20px' }}>
                <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.4)', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 6 }}>participate</div>
                <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.32)', marginBottom: 16, lineHeight: 1.6 }}>
                  Enter your Discord username below. Make sure it's exact so we can reach you if you win.
                </div>
                <div style={{ display: 'flex', gap: 8 }} className="enter-row">
                  <input
                    value={username}
                  ...
                  <button type="button"
                    ...
                    style={{
                      ...
                      whiteSpace: 'nowrap', letterSpacing: '0.06em', transition: 'background 0.2s, color 0.2s, border-color 0.2s, opacity 0.2s, transform 0.2s',
                    }}
                  >
                    {entering ? '…' : '🎲 join'}
                  </button>
                ...
                {msg && (
                  <div style={{ ...S, fontSize: 12, marginTop: 10, color: msg.ok ? '#4ade80' : 'var(--primary)', letterSpacing: '0.04em' }}>
                    {msg.ok ? '✓ ' : '⚠ '}{msg.text}
                  </div>
                )}
              </div>
            ...
            {alreadyEntered && (
              <div style={{ background: 'rgba(34,197,94,0.07)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: 16, padding: '28px 20px', textAlign: 'center', animation: 'fadeInUp 0.4s ease both' }}>
                ...
                <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)', letterSpacing: '0.08em' }}>good luck in the giveaway ✦</div>
              </div>
            )}
            ...
            {!isActive && raffle.entries.length > 0 && (
              <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 16, padding: '18px 20px' }}>
                <div style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5, maxHeight: 240, overflowY: 'auto' }}>
                  {raffle.entries.map((e, i) => (
                    <div key={i} style={{
                      ...
                      <span style={{ fontSize: 12, color: 'rgba(254,240,244,0.2)' }}>
            ...
            {isActive && raffle.entries.length > 0 && (
              <div style={{ background: 'rgba(255,255,255,0.015)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: 16, padding: '16px 20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                  ...
                  <span style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                    {raffle.entries.length} joined so far
                  </span>
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                  {raffle.entries.slice(-12).map((e, i) => (
                    <span key={i} style={{ ...S, fontSize: 12, padding: '3px 10px', borderRadius: 20, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', color: 'rgba(254,240,244,0.45)' }}>
                      {e.discordUsername}
                    </span>
                  ))}
                  {raffle.entries.length > 12 && (
                    <span style={{ ...S, fontSize: 12, padding: '3px 10px', borderRadius: 20, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', color: 'rgba(254,240,244,0.25)' }}>
