    ...
    <div style={{ position: 'relative' }}>
      <div style={{ display: 'flex', gap: 2 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, marginRight: 4 }}>
          ...
          {DAYS.map((d, i) => (
            <div key={i} style={{ height: 14, display: 'flex', alignItems: 'center', ...S, fontSize: 12, color: 'rgba(254,240,244,0.3)' }}>{d}</div>
          ))}
        </div>
        <div style={{ flex: 1 }}>
          {/* Hour labels */}
          <div style={{ display: 'flex', gap: 2, marginBottom: 2 }}>
            {Array.from({ length: 24 }, (_, h) => (
              <div key={h} style={{ flex: 1, textAlign: 'center', ...S, fontSize: 12, color: 'rgba(254,240,244,0.25)' }}>
                {h % 6 === 0 ? `${h}h` : ''}
              </div>
            ))}
          </div>
      ...
      {tip && tip.count > 0 && (
        <div style={{
          ...
          ...S, fontSize: 12, color: '#fee0f4', marginBottom: 6,
        }}>
      ...
      {top.map((p, i) => {
        ...
        return (
          <div key={p.page} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.35)', flex: '0 0 18px', textAlign: 'right' }}>{i + 1}</span>
            <div style={{ flex: `0 0 ${w}%`, height: 28, background: `rgba(196,20,40,${0.5 - i * 0.07})`, borderRadius: '0 4px 4px 0', display: 'flex', alignItems: 'center', paddingLeft: 10, transition: 'flex 0.4s ease' }}>
              <span style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.9)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.page}</span>
            </div>
            <span style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.4)' }}>{p.count}</span>
          </div>
        )
      })}
    </div>
        ...
        <Sec title="traffic by page — 7 days">
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={areaData} margin={{ top: 4, right: 8, bottom: 0, left: -24 }}>
              <XAxis dataKey="date" tick={{ fill: 'rgba(254,240,244,0.3)', fontSize: 12, fontFamily: 'Space Mono, monospace' }} tickLine={false} axisLine={false} />
              <YAxis tick={{ fill: 'rgba(254,240,244,0.3)', fontSize: 12 }} tickLine={false} axisLine={false} allowDecimals={false} />
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 10 }}>
            {topPages.map((p, i) => (
              <div key={p} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <span style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.45)' }}>{p}</span>
              </div>
            ))}
          </div>
        </Sec>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['source', 'visits', '%'].map(h => (
                    <th key={h} style={{ textAlign: 'left', ...S, fontSize: 12, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(196,20,40,0.55)', paddingBottom: 10, paddingRight: 16, borderBottom: '1px solid rgba(196,20,40,0.1)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {stats.byReferrer.map(r => {
                  ...
                  return (
                    <tr key={r.referrer}>
                      <td style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.6)', padding: '7px 16px 7px 0', borderBottom: '1px solid rgba(255,255,255,0.03)', maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</td>
                      <td style={{ ...S, fontSize: 12, color: 'var(--text)', padding: '7px 16px 7px 0', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{r.count}</td>
                      <td style={{ ...S, fontSize: 12, color: 'rgba(254,240,244,0.35)', padding: '7px 0', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{Math.round(r.count / stats.total * 100)}%</td>
                    </tr>
                  )
                })}
              </tbody>
