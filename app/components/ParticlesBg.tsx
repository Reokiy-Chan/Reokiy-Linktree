...
export default function ParticlesBg() {
  ...
  useEffect(() => {
    ...
    const spawn = () => {
      const color = palette.particleColors[Math.floor(Math.random() * palette.particleColors.length)]
      particles.push({
        x: Math.random() * canvas.width, y: canvas.height + 10,
        vx: (Math.random() - 0.5) * 0.5, vy: -(Math.random() * 0.8 + 0.2),
        size: Math.random() * 2.5 + 0.5, opacity: 0, color,
    ...
    const tick = () => {
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        p.opacity = progress < 0.1 ? progress * 10 * 0.7 : progress > 0.8 ? (1 - progress) * 5 * 0.7 : 0.7
